...
### Selling access outside Fayne

If you sell access somewhere else (your own checkout, a coaching package, a membership site) and grant courses by hand, a members-only course with no price and no offer would show students a dead end. Set a **custom access link** instead: open **Sell → Settings** and, under **Members-only courses**, enter a button label (for example "Get access") and an `https` URL. Locked course pages then show that button in place of a buy button, sending students to wherever they can get in. It applies academy-wide to every course that has no price and no advertised offer, and it saves as you type.

<Note>
  In academies that sell anything, free courses show a **Free** chip in the catalog so visitors can tell them apart from paid ones. Academies where everything is free stay unchanged.
</Note>
