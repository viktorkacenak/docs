...
When a student buys from you, whether a single paid course, a bundle, or the Academy Pass (see [Offers](/payments/offers)), they go through Stripe Checkout, a hosted payment page served directly by Stripe. You do not build or maintain a payment form. Stripe handles card validation, 3D Secure authentication, and international card support automatically.
...
Your owner dashboard includes a revenue summary in two places: **Offers → Sales** (the per-transaction ledger, with CSV export) and **Insights** (trends and per-course breakdowns).
