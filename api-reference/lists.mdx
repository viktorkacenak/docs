...
- **Drive course access.** When an offer in your academy grants its courses to a list, every member of that list can access those courses. Add someone to the list and they get access; remove them and the access is revoked. You configure offers in your dashboard; this API manages the list and its membership. For the start-to-finish recipe (lock courses, create a hidden free offer, grant it to a list, drive membership from here), see [Access Workflows](/api-reference/guides).

<Note>
  List access is **dynamic** and follows membership. This is different from an [enrollment](/api-reference/enrollments), which is a direct per-student course assignment. Use a list for cohort or tier access you may want to revoke later; use an enrollment for an individual assignment.
</Note>
...
curl -X POST https://yourname.fayneos.com/api/v1/lists \
  ...
  -d '{ "name": "VIP Cohort", "description": "Paying members" }'
```
  ...
  <Expandable title="properties" defaultOpen>
    <ResponseField name="courses" type="object[]" required>
      <Expandable title="course properties" defaultOpen>
        ...
        <ResponseField name="granted_via" type="object[]" required>
          The offers through which this list grants the course.

          <Expandable title="granted_via properties">
            ...
            <ResponseField name="role" type="string" required>
              How the list is attached: `grants_to` (the offer is free for this list's members) or `member` (the list is the offer's member list).
            </ResponseField>
          </Expandable>
        </ResponseField>
      </Expandable>
    ...
    "courses": [
      {
        ...
        "granted_via": [
          {
            "offer_id": "d4e5f6a7-b8c9-0123-def0-456789abcdef",
            "offer_name": "VIP Bundle",
            "role": "grants_to"
          }
        ]
      }
