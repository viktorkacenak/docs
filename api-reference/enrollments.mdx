...
Removes a student's access to a single course by revoking the enrollment. The enrollment record is kept but its access status is set to `revoked`, so the student loses access while the enrollment record is preserved. Re-enrolling later through [POST /api/v1/students/:studentId/enrollments](/api-reference/enrollments) restores access on the same record.
