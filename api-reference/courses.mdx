...
The courses endpoint returns every published course in your academy in display order. You can use this to build custom catalog pages or sync course data to an external system. Only published courses are returned — drafts are never included.
    ...
    <ResponseField name="courses" type="object[]" required>
      ...
      <Expandable title="course properties" defaultOpen>
        ...
        <ResponseField name="visibility" type="string" required>
          Whether the course is listed in the academy catalog. Either `published` (listed) or `hidden`.
        </ResponseField>
