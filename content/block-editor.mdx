...
description: "Build rich, interactive lessons with Fayne's visual block editor. Choose from nearly 30 block types covering text, media, quizzes, flashcards, and more."
icon: "layout-panel-left"
---

The block editor is where you write and arrange your lesson content. It is built on TipTap, a modern rich-text foundation, and gives you nearly 30 block types to work with. Every block can be dragged to a new position using the drag handle that appears on the left side when you hover over it. To insert any block, type `/` anywhere in the editor to open the slash command menu and search by name.
