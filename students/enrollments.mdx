...
You can also attach courses and offers to an [invitation](/students/managing-students#inviting-students), so new students arrive with their access already in place. Course grants can also be automated through the [API](/api-reference/enrollments); offers are granted from the dashboard.
...
<CardGroup cols={2}>
  ...
  <Card title="Continue where you left off" icon="play">
    A prominent prompt links directly to the next lesson the student can open, so they can pick up without searching.
  </Card>
...
## Pacing inside a course

By default, every lesson in a course a student can open is available from day one, in any order. Two per-course settings change that:

- [**Drip scheduling**](/content/drip-scheduling) releases lessons on a timer, either a set number of days after each completed lesson or on a fixed schedule from the student's start date.
- [**Knowledge checks**](/content/knowledge-checks) that are marked required lock the lessons after them until the student passes.

Both show locked lessons in the sidebar with the reason, and both are recalculated live, so changing a setting takes effect on the student's next visit.

## The course viewer

Inside a course, students see a full-screen reading experience. The lesson sidebar on the left lists every lesson in order, grouped under [module](/content/modules) headers when the course has them. Completed lessons display a checkmark, and the current lesson is highlighted.
...
<Tip>
  Without drip or required checks, students are not locked into a linear path. They can click any lesson in the sidebar to jump directly to it.
</Tip>
