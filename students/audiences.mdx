...
Some academies also have offers wired directly to a list, so that everyone on the list holds the offer for as long as they are a member (access rules configured before the offer model, or set up with Fayne's help). Where that is in place, membership drives access: add someone and the covered courses unlock; remove them and it ends. Making this self-serve on the offer editor is planned.
