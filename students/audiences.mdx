...
Lists are most powerful when they drive access, and they do it through [Offers](/payments/offers). An offer (a bundle, the Academy Pass, or a single-course offer) can name one of your Lists as the group it is **free for**. From then on, access follows list membership: add a student to the list and every course the offer covers unlocks for them; remove them and it ends. This lets you run cohort or tier access in one place instead of granting people course by course.

You set this up on the offer, in the Offers hub, by pointing it at the list. A typical pattern: a "VIP clients" list that holds your flagship bundle, maintained by hand here or synced from your CRM through the [Lists API](/api-reference/lists).

<Warning>
  Because a list can carry access, deleting one ends whatever its members were getting through it. The delete dialog warns you before you confirm.
</Warning>

Two related mechanisms also live on offers rather than lists: an offer can be **buyable only by** a specific list (group pricing, for example an alumni discount), and offers students purchase manage their own member lists automatically. See [Managing Students](/students/managing-students) for access and progress.
...
<CardGroup cols={2}>
  <Card title="Course access (Lists only)" icon="shield-check">
    Make an [offer](/payments/offers) free for a List, so membership drives who holds it and the courses it covers. Segments cannot gate access; they are for messaging and insight.
  </Card>

  <Card title="Newsfeed posts" icon="megaphone">
    In the [Newsfeed](/engagement/newsfeed) composer, send a post to **Everyone** or scope it to specific Lists or Segments.
  </Card>
</CardGroup>
