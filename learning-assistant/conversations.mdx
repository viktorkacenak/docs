...
<Steps>
  <Step title="Go to Learning Assistant">
    Click **Learning Assistant** in the owner sidebar. It opens the conversation inbox for the academy you are currently in.
  </Step>
...
<CardGroup cols={2}>
  ...
  <Card title="Search" icon="search">
    A **Search conversations** field that filters the list as you type, matching on student name, email, conversation title, and the latest message preview.
  </Card>
</CardGroup>
...
- **Student name and email.** Click the name or avatar to open the same student detail view you get from the [People](/students/managing-students) page, so you can check enrollment and progress without leaving the inbox.
...
The assistant flags moments a human should follow up on. When a student asks about pricing, signals they are ready to buy, gets stuck, or asks for a person, the assistant raises a flag on that turn and the conversation joins the **Needs you** filter at the top of the list (the tab shows the count). Each flagged turn carries a chip naming the reason:
...
Read the exchange, reach out to the student however you normally would, then click **Mark solved** on the chip to clear it. A solved flag can be reopened. Once every flag in a conversation is solved, it drops out of **Needs you**.

### Conversation summary

Above longer transcripts, the assistant keeps a short running summary: the student's **goal**, what has been **established** so far, and the **open** questions still on the table. It is the same context the assistant uses to keep a multi-day conversation coherent, and it lets you catch up on a thread without reading every message.
...
Each Learning Assistant reply uses **1 AI credit** from your academy's shared pool. If the pool is fully used, the assistant pauses until the next period starts or you move to a plan with a larger allowance. Existing conversations stay readable while replies are paused. To bound spend per learner regardless of the pool, set a per-student daily message cap in [Customization](/learning-assistant/customization).
