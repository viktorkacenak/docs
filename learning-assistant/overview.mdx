...
<AccordionGroup>
  <Accordion title="Does it ever use lessons a student hasn't unlocked?">
    No. The assistant only reads published lessons in the course the student is in, and their access to that course is verified live (the same check that gates the course itself) before any answer is generated. Drafts are never used.
  </Accordion>
