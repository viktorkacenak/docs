...
If you have turned on [welcome questions](/academy/welcome-questions), the assistant reads that member's answers (their role, where they are with the topic, what is in their way, and their goal in their own words) alongside the course they are in. It can then speak to the member's actual situation instead of explaining generically. A member with no answers gets exactly the assistant they always had, and members are told the assistant uses their answers.
...
<Warning>
  The assistant goes quiet whenever an academy is paused (for example, a lapsed trial or plan). It also pauses if your [AI credit pool](/billing/plans#ai-credits) for the period runs out: each reply to a student costs 1 credit, and when the pool is empty students see a note that the assistant is taking a break. Existing transcripts stay readable, but students cannot start new turns until the academy is active again.
</Warning>
