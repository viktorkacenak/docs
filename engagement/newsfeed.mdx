...
<Steps>
  ...
  <Step title="Send it by email too (optional)">
    Turn on the mail toggle in the composer to email the post to its audience as well. It is off by default, decided per post. The email goes out from your academy's identity as a teaser: the title, a plain-text excerpt, and a link to the Feed, so students read the full post inside your academy. Targeted posts email exactly the people the audience picker counted at publish time. A post is emailed at most once, even if you unpublish and republish it. Students can turn these announcement emails off in their preferences without affecting receipts or course access mail.
  </Step>
...
Published posts appear instantly in your feed management list and on your students' Feed page.
...
Targeted posts are resolved when you publish (or, for scheduled posts, when you schedule them): the post reaches whoever qualified at that point in time, which is the right behavior for a point-in-time announcement. To reach a different group, post again.
