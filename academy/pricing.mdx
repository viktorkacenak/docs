...
<Note>
  Connecting Stripe requires a paid plan (Pro or Scale); trial academies cannot connect Stripe. You need a connected Stripe account to sell, so if everything in your academy is free, you can skip the Stripe connection.
</Note>
...
You set a course's access (free, paid, or members-only) in that course's settings, or for all courses at once from **Offers → Pricing**. See [Pricing Models](/payments/pricing-models) for how the three settings behave.
...
Your owner dashboard shows your revenue in two places: **Offers → Sales** is the transaction ledger (every purchase, refund, and dispute, with gross, fees, and net, plus CSV export), and **Insights** shows revenue trends and a per-course breakdown for the period you select. For full transaction history, disputes, and payout schedules, open your Stripe dashboard.

<AccordionGroup>
  <Accordion title="Can I change a course's price after launch?">
    Yes. Update it in the course's settings or the Offers → Pricing table at any time. Students who have already purchased keep their access; a price change only affects new purchases.
  </Accordion>
